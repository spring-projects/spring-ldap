...
<properties>
	<!-- ... -->
	<spring.version>{spring-framework-bom-version}</spring.version>
</properties>
----
...
[subs="verbatim,attributes"]
----
ext['spring.version']='{spring-framework-bom-version}'
----
======
...
Spring LDAP builds against Spring Framework 6 or higher but should generally work with any newer version of Spring Framework 5.x.
Many users are likely to run afoul of the fact that Spring LDAP's transitive dependencies resolve Spring Framework 6 or higher, which can cause strange classpath problems.
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework:spring-framework-bom:{spring-framework-bom-version}'
	}
}
----
======
...
The preceding example ensures that all the transitive dependencies of Spring LDAP use the Spring {spring-framework-bom-version} modules.
...
[[maven-repositories]]
=== Maven Repositories
Spring LDAP is deployed to Maven Central, so you need not declare additional Maven repositories in your build configuration.
