...
/**
 * Integration tests for the virtual list view search result capability of LdapTemplate.
 * The test should reflect the example in Chapter 7 of the
 * <a href= "https://www3.tools.ietf.org/html/draft-ietf-ldapext-ldapv3-vlv-09">Virtual
 * List View RFC draft</a>.
 *
 * <blockquote> Here we walk through the client-server interaction for a specific virtual
 * list view example: The task is to display a list of all 78564 persons in the US company
 * "Ace Industry". This will be done by creating a graphical user interface object to
 * display the list contents, and by repeatedly sending different versions of the same
 * virtual list view search request to the server. The list view displays 20 entries on
 * the screen at a time.
 * <p>
 * We form a search with baseObject of "o=Ace Industry,c=us"; scope of wholeSubtree; and
 * filter of "(objectClass=person)". We attach a server-side sort control [SSS] to the
 * search request, specifying ascending sort on attribute "cn". To this search request, we
 * attach a virtual list view request control with contents determined by the user
 * activity and send the search request to the server. We display the results from each
 * search result entry in the list window and update the slider position.
 * <p>
 * When the list view is first displayed, we want to initialize the contents showing the
 * beginning of the list. Therefore, we set beforeCount to 0, afterCount to 19,
 * contentCount to 0, offset to 1 and send the request to the server. The server duly
 * returns the first 20 entries in the list, plus a content count of 78564 and
 * targetPosition of 1. We therefore leave the scroll bar slider at its current location
 * (the top of its range).
 * <p>
 * Say that next the user drags the scroll bar slider down to the bottom of its range. We
 * now wish to display the last 20 entries in the list, so we set beforeCount to 19,
 * afterCount to 0, contentCount to 78564, offset to 78564 and send the request to the
 * server. The server returns the last 20 entries in the list, plus a content count of
 * 78564 and a targetPosition of 78564.
 * <p>
 * Next the user presses a page up key. Our page size is 20, so we set beforeCount to 0,
 * afterCount to 19, contentCount to 78564, offset to 78564-19-20 and send the request to
 * the server. The server returns the preceding 20 entries in the list, plus a content
 * count of 78564 and a targetPosition of 78525.
 * <p>
 * Now the user grabs the scroll bar slider and drags it to 68% of the way down its
 * travel. 68% of 78564 is 53424 so we set beforeCount to 9, afterCount to 10,
 * contentCount to 78564, offset to 53424 and send the request to the server. The server
 * returns the preceding 20 entries in the list, plus a content count of 78564 and a
 * targetPosition of 53424.
 * <p>
 * Lastly, the user types the letter "B". We set beforeCount to 9, afterCount to 10 and
 * greaterThanOrEqual to "B". The server finds the first entry in the list not less than
 * "B", let's say "Babs Jensen", and returns the nine preceding entries, the target entry,
 * and the proceeding 10 entries. The server returns a content count of 78564 and a
 * targetPosition of 5234 and so the client updates its scroll bar slider to 6.7% of full
 * scale. </blockquote>
 *
