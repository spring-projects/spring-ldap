...
public final class CompensatingTransactionUtils {
	...
	/**
	 * Perform the specified operation, storing the state prior to the operation in order
	 * to enable commit/rollback later. If no transaction is currently active, proceed
	 * with the original call on the target.
	 * @param synchronizationKey the transaction synchronization key we are operating on
	 * (typically something similar to a DataSource).
	 * @param target the actual target resource that should be used for invoking the
	 * operation on should no transaction be active.
	 * @param method name of the method to be invoked.
	 * @param args arguments with which the operation is invoked.
	 */
